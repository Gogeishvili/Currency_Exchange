...
class CurrencyExchangeApp:
    def __init__(self, converter, currency_data, height, width):
        ...
        self.__create_root(height, width)
        self.__create_widgets()

    def run(self):
        self.__root.mainloop()

    def __create_root(self, height, width):
    ...
    def __create_widgets(self):
        ...
        button_convert = tk.Button(self.__root, text="Convert", command=self.__convert_currency)
        button_convert.grid(row=4, column=0, padx=10, pady=20)

        button_reset = tk.Button(self.__root, text="Reset", command=self.__reset_fields)
